...
class Mixup(object):
    ...
    def __init__(self, prob=0.5, alpha=1.5, mixup=False,
                 pkl_path='data/coco/annotations/pgtrainval2017.json',
        ...
        all_labels = mmcv.load(self.pkl_path)
        self.all_labels = all_labels

    def get_img2(self):
        # random get image2 for mixup

        idx2 = np.random.choice(np.arange(len(self.all_labels)))
        img2_fn = self.all_labels[idx2]['filename']
        img2_path = self.img_path + img2_fn
        img2 = cv2.imread(img2_path)

        # get image2 label
        labels2 = self.all_labels[idx2]['ann']['labels']
        boxes2 = self.all_labels[idx2]['ann']['bboxes']
    ...
    def __call__(self, results):
        if self.mixup == True:
            if random.uniform(0, 1) > self.prob:
                ...
                if labels2 == []:
                    lambd = 0.9  # float(round(random.uniform(0.5,0.9),1))
                    # mix image
                    mixup_image = np.zeros([height, width, 3], dtype='float32')
                    mixup_image[:img1.shape[0], :img1.shape[1], :] = img1.astype('float32') * lambd
                    mixup_image[:img2.shape[0], :img2.shape[1], :] += img2.astype('float32') * (1. - lambd)
                    ...
                    mixup_image[:img2.shape[0], :img2.shape[1], :] += img2.astype('float32') * (1. - lambd)
                    mixup_image = mixup_image.astype('uint8')
                    results["gt_labels_weights"] = np.array([lambd for _ in labels1] + [1 - lambd for _ in labels2])
                    # mix labels
                ...
                results["gt_labels_weights"] = np.array([1. for _ in results['gt_labels']])
                return results
