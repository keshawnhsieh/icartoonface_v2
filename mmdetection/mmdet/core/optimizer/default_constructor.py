...
class DefaultOptimizerConstructor(object):
    ...
    def __call__(self, model):
        ...
        if not self.paramwise_cfg:
            ...
            swa_config = optimizer_cfg.pop('swa', None)

            base_optimizer = build_from_cfg(optimizer_cfg, OPTIMIZERS)

            if swa_config:
                # insert swa if specified
                swa_optimizer = torchcontrib.optim.SWA(base_optimizer)
                return swa_optimizer
            else:
